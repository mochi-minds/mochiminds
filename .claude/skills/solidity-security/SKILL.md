...
contract SecureContract is Ownable {
    constructor() Ownable(msg.sender) {} // OZ v5 requires initialOwner
...
import {SecureToken} from "../src/SecureToken.sol";
import {SecureContract} from "../src/SecureContract.sol";
...
contract SecurityTest is Test {
    ...
    function test_RevertWhen_ReentrancyAttack() public {
        ...
        // Reentrancy should be blocked by ReentrancyGuard
        vm.expectRevert();
        // ... attack contract call
    }

    function test_RevertWhen_IntegerOverflow() public {
        vm.expectRevert();
        token.transfer(attacker, type(uint256).max);
    }

    function test_RevertWhen_UnauthorizedAccess() public {
        // OZ v5 uses custom error OwnableUnauthorizedAccount
        vm.prank(attacker);
        vm.expectRevert(
            abi.encodeWithSignature("OwnableUnauthorizedAccount(address)", attacker)
        );
        secureContract.withdraw(100);
    }
}
```
