...
> **Next.js 15+:** `fetch` defaults to `no-store` (no caching). You must explicitly opt in to caching with `force-cache` or `next.revalidate`.
...
// Cache forever (static) — must opt in
